...
APP?=microssy
PROJECT=github.com/lyakhov/microssy
...
MICROSSY_LOCAL_HOST?=0.0.0.0
MICROSSY_LOCAL_PORT?=8080
MICROSSY_LOG_LEVEL?=0
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${MICROSSY_LOCAL_PORT}:${MICROSSY_LOCAL_PORT} \
		-e "MICROSSY_LOCAL_HOST=${MICROSSY_LOCAL_HOST}" \
		-e "MICROSSY_LOCAL_PORT=${MICROSSY_LOCAL_PORT}" \
		-e "MICROSSY_LOG_LEVEL=${MICROSSY_LOG_LEVEL}" \
